...
function setupPokemonStats(pokemon) {
  let poke = pokemon.stats;
  let total = 0;

  poke.forEach((stat) => (total += stat.base_stat));

  document.querySelector('.stats').innerHTML = `
  
      <div class="current-pokemon-stats-container column">
        <div class="current-pokemon-stats-name hp">
          HP
        </div>
        <h5 id="current-pokemon-stats-hp">${pokemon.stats[0].base_stat}</h5>
      </div>

      <div class="current-pokemon-stats-container column">  
        <div class="current-pokemon-stats-name atk">
        ATK
        </div>
        <h5 id="current-pokemon-stats-atk">${pokemon.stats[1].base_stat}</h5>
      </div>

    <div class="current-pokemon-stats-container column">
      <div class="current-pokemon-stats-name def">
        DEF
      </div>
      <h5 id="current-pokemon-stats-def">${pokemon.stats[2].base_stat}</h5>
    </div>

    <div class="current-pokemon-stats-container column">
      <div class="current-pokemon-stats-name sp-a">
        SpA
      </div>
      <h5 id="current-pokemon-stats-spa">${pokemon.stats[3].base_stat}</h5>
    </div>

    <div class="current-pokemon-stats-container column">
      <div class="current-pokemon-stats-name sp-d">
        SpD
      </div>
      <h5 id="current-pokemon-stats-spd">${pokemon.stats[4].base_stat}</h5>
    </div>

    <div class="current-pokemon-stats-container column">
      <div class="current-pokemon-stats-name spd">
        SPD
      </div>
      <h5 id="current-pokemon-stats-speed">${pokemon.stats[5].base_stat}</h5>
    </div>

    <div
      class="current-pokemon-stats-container column ">
      <div class="current-pokemon-stats-name total">
        TOT
      </div>
      <h5 id="current-pokemon-stats-total">${total}</h5>
    </div>
  `;
}
...
function setupEvolution(chain, no) {
  ...
  if (chain.evolves_to[0].evolution_details[0].min_level) {
    ...
    chainLevels[no].innerHTML = 'Trade';
  }
}
...
document.addEventListener('DOMContentLoaded', getAllNames);
closeInfoBtn.addEventListener('click', closePokemonInfo);
